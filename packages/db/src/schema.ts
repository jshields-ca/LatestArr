...
export const newsletters = sqliteTable("newsletters", {
  ...
  // list — distinct from the auto-generated "Here's what's new in the
  // last N days" line. Ignored once a custom templateId is set, same as
  // emailFont above.
  introText: text("intro_text"),
  // Free-text shown near the bottom, above the "Generated by LatestArr"
  // line. Same default-template-only scope as introText.
  footerNote: text("footer_note"),
  // Up to a handful of {label, url} call-to-action buttons rendered in the
  // default template (e.g. a fixed link to the Plex/Jellyfin app, or a
  // "browse the library" link) — capped at 4 by the create/update zod
  // schema in apps/server/src/http/routes/newsletters.ts, not here.
  ctas: text("ctas", { mode: "json" }).$type<{ label: string; url: string }[]>(),
  ...timestamps,
});
